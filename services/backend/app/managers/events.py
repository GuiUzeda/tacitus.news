...
class EventsManager(BaseManager):
    ...
    def search_events(
        ...
        stmt = select(NewsEventModel).filter(
            NewsEventModel.search_vector_ts.op("@@")(func.websearch_to_tsquery("portuguese", query))
        ).order_by(
            func.ts_rank_cd(NewsEventModel.search_vector_ts, func.websearch_to_tsquery("portuguese", query)).desc(), 

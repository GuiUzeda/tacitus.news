...
from models import (
    ArticlesQueueModel, EventsQueueModel, # Separate queues
    JobStatus, EventsQueueName
)
# Shared Lib Models

from news_events_lib.models import (
    ArticleModel, NewsEventModel, MergeProposalModel
)
...
class EditorialCLI:
    ...
    def start(self):
        while True:
            console.clear()
            console.print(Panel.fit("📰 Tacitus Editorial Control Room", style="bold blue"))
            ...
            if choice == "1": self.review_merges()
            elif choice == "2": self.queue_manager()
            elif choice == "3": self.manual_search()
            elif choice == "4": self.inspect_tool()
            elif choice == "5": self.merge_duplicate_events()
            elif choice == "q":
                console.print("Bye! 👋")
                sys.exit(0)
    ...
    def review_merges(self):
        with SessionLocal() as session:
            stmt = (
                ...
                .order_by(MergeProposalModel.source_article_id, MergeProposalModel.similarity_score)
            )
        ...
        grouped_props = {k: list(v) for k, v in groupby(all_proposals, key=lambda x: x.source_article) if k is not None}
        ...
        for article, props in grouped_props.items():
            current_idx += 1
            if self._handle_single_review(article, props, current_idx, total_groups) == 'quit':
                break
    ...
    def _handle_single_review(self, article, props, idx, total):
        while True:
            ...
            console.print(table)
            console.print("\n[bold]Actions:[/bold]")
            console.print("[green][#][/green] Accept Candidate   [blue][i #][/blue] Inspect   [blue][s][/blue] Search DB")
            console.print("[red][n][/red] New Event (Reject All) [dim][skip][/dim] Skip      [dim][q][/dim] Quit")
    ...
    def _execute_merge(self, selected_prop, all_props_for_article):
        with SessionLocal() as session:
            ...
            if not article or not event:
                console.print("[red]Error: Source Article or Target Event not found in DB.[/red]")
                return
            
            # 2. Logic: Link Article to Event
            self.cluster_service._link_to_event(session, event, article, article.embedding)
            ...
            queue_item = session.execute(
                select(ArticlesQueueModel).where(ArticlesQueueModel.article_id == article.id)
            ).scalar_one_or_none()
    ...
    def manual_search(self, article=None):
        ...
        with SessionLocal() as session:
            if not article:
                ...
                query = Prompt.ask("Refine Query", default=self.cluster_service.derive_search_query(article))
            ...
            if article:
                ...
                if choice.isdigit() and 1 <= int(choice) <= len(candidates):
                    ...
                    if not art_db or not ev_db:
                        console.print("[red]Error: Article or Event not found in DB.[/red]")
                        return
                    
                    self.cluster_service._link_to_event(session, ev_db, art_db, art_db.embedding)
            ...
            if choice == 'r':
                with SessionLocal() as session:
                    ...
                    res1 = session.execute(
                        update(ArticlesQueueModel).where(ArticlesQueueModel.status == JobStatus.FAILED).values(status=JobStatus.PENDING, msg=None)
                    )
                    res2 = session.execute(
                        update(EventsQueueModel).where(EventsQueueModel.status == JobStatus.FAILED).values(status=JobStatus.PENDING, msg=None)
                    )
                    session.commit()
                    console.print(f"[green]Retried {res1.rowcount} Articles, {res2.rowcount} Events.[/green]")
                    time.sleep(1)
            if choice == 'c':
                with SessionLocal() as session:
                    session.execute(delete(ArticlesQueueModel).where(ArticlesQueueModel.status == JobStatus.COMPLETED))
                    session.execute(delete(EventsQueueModel).where(EventsQueueModel.status == JobStatus.COMPLETED))
                    session.commit()
    ...
    def inspect_tool(self):
        ...
        with SessionLocal() as session:
            try:
                ...
                events = session.query(NewsEventModel).filter(NewsEventModel.title.ilike(f"%{event_id_str}%")).limit(5).all()
                if not events:
                ...
                table.add_column("#"); table.add_column("Title"); table.add_column("ID")
    ...
    def _inspect_event_deep_dive(self, event_id):
        while True:
            console.clear()
            with SessionLocal() as session:
                event = session.query(NewsEventModel).options(
                ...
                if event.summary:
                    ...
                    if isinstance(event.summary, dict):
                        sum_text = event.summary.get('center', '') or event.summary.get('bias', '')
                    console.print(Panel(sum_text, title="Summary", border_style="green"))

                table = Table(title="Linked Articles", show_header=True)
                table.add_column("#", width=4); table.add_column("Title"); table.add_column("Date")
                sorted_articles = sorted(event.articles, key=lambda x: x.published_date or datetime.min, reverse=True)
                for i, art in enumerate(sorted_articles):
                    table.add_row(str(i+1), art.title, str(art.published_date.date() if art.published_date else '?'))
                console.print(table)
    ...
    def _read_article_content(self, article):
        ...
        console.print(f"[dim]{article.original_url}[/dim]\n")
        if article.summary: console.print(Panel(article.summary, title="Summary"))
    ...
    def merge_duplicate_events(self):
        ...
        with SessionLocal() as session:
            targets = session.query(NewsEventModel).filter(
                NewsEventModel.title.ilike(f"%{target_query}%"), NewsEventModel.is_active == True
            ).limit(10).all()
            if not targets:
                console.print("[red]No events found.[/red]")
                time.sleep(2); return
            ...
            table = Table(title="Potential Duplicates"); table.add_column("#"); table.add_column("Title"); table.add_column("Score")
            ...
            to_merge_indices = []
            if choice == 'all': to_merge_indices = range(len(candidates))
            else:
                try: to_merge_indices = [int(x.strip())-1 for x in choice.split(',') if x.strip()]
                except: return

            if not to_merge_indices: return
            if Confirm.ask(f"⚠️ Merge {len(to_merge_indices)} events?"):
                for idx in to_merge_indices:
                    if 0 <= idx < len(candidates):
                        self._execute_event_merge(session, candidates[idx][0], target_event)
                session.commit()
                console.print("[green]Merged![/green]")
                time.sleep(2)
    ...
    def _execute_event_merge(self, session, source_ev, target_ev):
        ...
        session.query(MergeProposalModel).filter(or_(MergeProposalModel.target_event_id == source_ev.id)).update({"status": "rejected"})
        
        # Trigger re-enhancement for target
        new_job = EventsQueueModel(
            event_id=target_ev.id, queue_name=EventsQueueName.ENHANCER, status=JobStatus.PENDING,

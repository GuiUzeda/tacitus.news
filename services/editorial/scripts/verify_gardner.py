...
async def verify_gardner_tasks():
    """
    Verifies that GardnerService correctly handles:
    ...
    try:
        logger.info("🚜 Running Gardner Cycle...")
        # Run the service
        with SessionLocal() as session:
            await gardner.run_cycle(session)
        ...
        with SessionLocal() as session:
            ...
            if e_stale.status == EventStatus.ARCHIVED and not e_stale.is_active:
                ...
                logger.error(f"❌ Archivist Failed: Status={e_stale.status}, Active={e_stale.is_active}")

            # Verify Score Decay
            e_decay = session.get(NewsEventModel, decay_id)
            if e_decay.hot_score < 999.0:
                logger.success(f"✅ Score Decay: Score reduced from 999.0 to {e_decay.hot_score}")
            else:
        ...
        with SessionLocal() as session:
            session.query(NewsEventModel).filter(NewsEventModel.id.in_([stale_id, decay_id])).delete(synchronize_session=False)
            session.query(EventsQueueModel).filter(EventsQueueModel.id == queue_id).delete(synchronize_session=False)
            session.commit()

if __name__ == "__main__":
    asyncio.run(verify_gardner_tasks())
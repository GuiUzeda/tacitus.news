...
# Add service root to path to allow imports from core
sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

from core.browser import BrowserFetcher
from core.llm_parser import CloudNewsAnalyzer
from config import Settings
from news_events_lib.models import NewspaperModel, ArticleModel
            ...
            async with session.get(url, timeout=30) as response:
                if response.status == 200:
                    ...
                    try:
                        ...
                        logger.error(f"Browser fetch fallback failed for {url}: {browser_e}")
                        return None
...
async def test_date_extraction(html: str, url: str):
    ...
    has_strong_signal = [
        ...
        bs4_html.find('meta', attrs={'property': re.compile(r'date|time|published', re.I)}) ,
    ...
    if any(has_strong_signal) and not date:
        logger.info(f"Found strong date signal. Trusting htmldate...  {has_strong_signal}")
        try:
            date = find_date(html, original_date=True, outputformat='%Y-%m-%dT%H:%M:%S', extensive_search=True)
    ...
    if not date:
        ...
        candidates = bs4_html.find_all(['div', 'span', 'p', 'time', 'li'], class_=date_class)

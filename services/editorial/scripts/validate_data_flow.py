...
from news_events_lib.models import NewsEventModel, ArticleModel, NewspaperModel, EventStatus
    ...
    with SessionLocal() as session:
        try:
            ...
            if event.article_count != 3:
                raise AssertionError(f"Aggregation Failed: Expected 3 articles, got {event.article_count}")
            if event.stance != -0.5:
                raise AssertionError(f"Aggregation Failed: Expected stance -0.5, got {event.stance}")
            ...
            if event.blind_spot_side != "left":
                raise AssertionError(f"Publishing Failed: Wrong blind spot side {event.blind_spot_side}")
            if "BLIND_SPOT" not in event.publisher_isights:
                raise AssertionError("Publishing Failed: Missing BLIND_SPOT tag")

            initial_score = event.hot_score
            logger.info(f"✅ Phase 2: Publishing Verified. Score: {initial_score}, BlindSpot: {event.blind_spot_side}")

            # --- PHASE 3: DECAY (GARDNER) ---
            # Simulate 48 hours passing
            event.first_article_date = datetime.now(timezone.utc) - timedelta(hours=48)
            session.commit()

            # Run Gardner Decay
            await gardner._run_score_decay(session)
            session.refresh(event)

            decayed_score = event.hot_score
            if decayed_score >= initial_score:
                raise AssertionError(f"Decay Failed: Score did not decrease ({initial_score} -> {decayed_score})")
            ...
            session.query(ArticleModel).filter(ArticleModel.event_id == event_id).delete()

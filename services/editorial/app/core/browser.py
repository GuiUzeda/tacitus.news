...
class BrowserFetcher:
    ...
    @classmethod
    async def fetch(
        cls, url: str, scroll_depth: int = 0, return_bytes: bool = False
    ) -> str:
        """
                    ...
                    try:
                        ...
                        if response:
                            ...
                            if "application/pdf" in ctype or "image/" in ctype:
                                # logger.warning(f"🚫 Browser detected binary ({ctype}): {url}")
                                await browser.close()
                                return "binary"
                            ...
                            for i in range(scroll_depth):
                                ...
                                try:
                                    load_btn = page.get_by_role(
                                        "button",
                                        name=re.compile(
                                            r"carregar mais|ver mais|leia mais|veja mais",
                                            re.IGNORECASE,
                                        ),
                                    )
                                    if (
                                        await load_btn.count() > 0
                                        and await load_btn.first.is_visible()
                                    ):

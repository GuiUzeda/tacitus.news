...
class ReviewWaitingMenu(Menu):
    ...
    def render(self):
        with SessionLocal() as session:
            ...
            if not self.events:
                return Panel(Align.center("[green]No events waiting for approval.[/green]"), title="Waiting Room")
...
class ReviewMergesMenu(Menu):
    ...
    def render(self):
        with SessionLocal() as session:
            a_count = session.scalar(
                select(func.count(MergeProposalModel.id))
                .where(MergeProposalModel.status == JobStatus.PENDING, MergeProposalModel.source_article_id.is_not(None))
            )
            e_count = session.scalar(
                select(func.count(MergeProposalModel.id))
                .where(MergeProposalModel.status == JobStatus.PENDING, MergeProposalModel.source_event_id.is_not(None))
            )
    ...
    def handle_input(self, choice):
        if choice == "q": return 'back'
        if choice == "1": self._review_article_merges()
        if choice == "2": self._review_event_merges()
        return None
    ...
    def _review_event_merges(self):
        while True:
            ...
            with SessionLocal() as session:
                proposal = session.scalars(
                    select(MergeProposalModel)
                    .options(joinedload(MergeProposalModel.source_event), joinedload(MergeProposalModel.target_event))
                    .where(MergeProposalModel.status == JobStatus.PENDING, MergeProposalModel.source_event_id.is_not(None))
                    .order_by(MergeProposalModel.distance_score)
                    .limit(1)
                ).first()
                ...
                console.print(f"[bold green]TARGET (Survivor):[/bold green] {target.title} ({target.article_count} arts)")

...
class EngineRoomMenu(Menu):
    ...
    def handle_input(self, choice):
        if choice == "q": return 'back'
        if choice == "r": return None
...
class EngineRoomActions:
    ...
    def run_backfill(self):
        ...
        script_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "scripts", "backfill_interests.py")
        try:
    ...
    def run_sql_console(self):
        ...
        console.print("[dim]Shortcuts: 'tables' to list tables, 'desc <table>' to show columns.[/dim]")
            ...
            try:
                with SessionLocal() as session:
                    ...
                    if result.returns_rows:
                        ...
                        session.commit()
                        console.print(f"[green]Executed. Rows affected: {result.rowcount}[/green]")
    ...
    def run_orm_console(self):
        ...
        try:
            ...
            readline = None
            console.print("[yellow]Readline not available. Autocomplete disabled.[/yellow]")
        ...
        ctx = {
            "session": session,
            "select": select, "func": func, "update": update, "delete": delete, "text": text,
        ...
        console.print("[dim]Interactive Shell. Objects: session, select, NewsEventModel, ArticleModel...[/dim]")
        console.print("[dim]Example: session.scalars(select(NewsEventModel).limit(5)).all()[/dim]")

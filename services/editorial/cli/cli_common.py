...
from news_events_lib.models import ArticleModel, NewsEventModel, ArticleContentModel, MergeProposalModel
...
def read_article_content(article):
    ...
    console.print(f"[link={article.original_url}]{article.original_url}[/link]", style="blue underline", no_wrap=True, overflow="ignore")
    print() 
...
def inspect_event_deep_dive(event_id):
    ...
    while True:
        console.clear()
        with SessionLocal() as session:
            event = session.query(NewsEventModel).options(joinedload(NewsEventModel.articles)).get(event_id)
            if not event: return

...
class EventManageMenu(Menu):
    ...
    def render(self):
        with SessionLocal() as session:
            ...
            info_table.add_row(f"ID: {event.id}", f"Status: [{event.status.value}]", f"Active: {event.is_active}")
            info_table.add_row(f"Articles: {event.article_count}", f"Published: {event.published_at}", "")
            info_table.add_row(f"Last Updated: {event.last_updated_at}", f"Last Article: {event.last_article_date}", "")
            ...
            return Panel(Group(info_table, score_panel, actions), title=f"Managing: {event.title}")

    def handle_input(self, choice):
        action = choice.lower()
        if action == "q": return 'back'
        
        with SessionLocal() as session:
            event = session.get(NewsEventModel, self.event_id)
            if not event: return 'back'
...
class LiveDeskMenu(Menu):
    ...
    def render(self):
        with SessionLocal() as session:
            ...
            if not self.events:
                return Panel("[yellow]No published events.[/yellow]", title="Live Feed Control")

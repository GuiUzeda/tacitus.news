...
export const BlindspotCard = ({ events }: { events: FeedEvent[] }) => {
  ...
  return (
      <div>{events.map((event) => {
        ...
        return (
        <Card key={event.id} className="rounded-sm border-2 border-dashed border-zinc-900 bg-zinc-50 shadow-none">
          <CardContent className="flex items-center gap-4 p-4">
            ...
            <div className="space-y-1">
              ...
              <p className="font-serif text-base font-medium leading-tight text-zinc-900">
                {event.title} <span className="text-zinc-400">— Ignorado pela {ignoredBy}.</span>
              </p>
            </div>
          </CardContent>
        </Card>

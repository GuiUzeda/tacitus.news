...
export const getFeedEvents = async (): Promise<FeedEvent[]> => {
  try {
    ...
    const feedEvents: FeedEvent[] = data.items.map((newsEvent) => ({
      ...
      biasDistribution: {
        left: newsEvent.article_counts_by_bias.left || 0 + newsEvent.article_counts_by_bias.leans_left || 0,
        center: newsEvent.article_counts_by_bias.center || 0,
        right: newsEvent.article_counts_by_bias.right || 0 + newsEvent.article_counts_by_bias.leans_right || 0,
      },
      ...
      stanceDistribution: {
        left: newsEvent.stance_distribution.left?.total || 0 + newsEvent.stance_distribution.leans_left?.total || 0,
        center: newsEvent.stance_distribution.center?.total || 0,
        right: newsEvent.stance_distribution.right?.total || 0 + newsEvent.stance_distribution.leans_right?.total || 0
      },
